...
class ReflectiveIcosahedron extends ReflectiveFacetedSolid {
    constructor(center,radius,color,tranformationMatrix) {
        ...
        if (tranformationMatrix) {
            if (!(tranformationMatrix instanceof Matrix3D)) {
                throw 'transformationMatrix is not an instance of Matrix3D'
            }
            vList = vList.map(vertex=>tranformationMatrix.vectorMult(vertex))
        }
        const fList = [
            [0,1,2],
            [0,2,3],
