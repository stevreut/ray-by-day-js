...
class OpticalEnvironment {
    ...
    colorFromRay(ray) {
        ...
        // TODO - atmospheric alteration here?
        const ATMOSK = 0.995
        const WASH_COLOR = [0.8,0.6,0.6]
        const washFactor = Math.max(0,Math.min(1,ATMOSK**leastDist))
        const washContraFactor = 1-washFactor
        const newColor = ray.color.map((prim,idx)=>prim*washFactor+WASH_COLOR[idx]*washContraFactor)
        ray.color = newColor
        // TODO - end atmos. code?
